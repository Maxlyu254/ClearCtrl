...
void PrintUsage(const char* prog) {
    std::cout
        ...
        << "  --log-path=FILE                     Output CSV path (default: metrics_<run_id>.csv)\n"
        << "  --run-id=ID                         Explicit run id (default: auto timestamp)\n"
        << "RocksDB options:\n"
        << "  --rocksdb-increase-parallelism=N    IncreaseParallelism (default: 2)\n"
        << "  --rocksdb-write-buffer-size-mb=N    write_buffer_size in MiB (default: 4)\n"
        << "  --rocksdb-l0-compaction-trigger=N   level0_file_num_compaction_trigger (default: 12)\n"
        << "  --rocksdb-l0-slowdown-trigger=N     level0_slowdown_writes_trigger (default: 20)\n"
        << "  --rocksdb-l0-stop-trigger=N         level0_stop_writes_trigger (default: 36)\n"
        << "ClearCtrl options:\n"
        ...
        << "  --ctrl-interval-sec=N               Control interval seconds (default: 5)\n"
        << "  --ctrl-cooldown-sec=N               Switch cooldown seconds (default: 5)\n"
        << "  -h, --help            Show help\n";
}
...
int main(int argc, char** argv) {
    ...
    int rocksdb_increase_parallelism = 2;
    uint64_t rocksdb_write_buffer_size_mb = 4;
    int rocksdb_l0_compaction_trigger = 12;
    int rocksdb_l0_slowdown_trigger = 20;
    int rocksdb_l0_stop_trigger = 36;

    int ctrl_low_threshold = 4;
    int ctrl_high_threshold = 10;
    int ctrl_low_bg_jobs = 2;
    int ctrl_high_bg_jobs = 6;
    int ctrl_interval_sec = 5;
    int ctrl_cooldown_sec = 5;
    ...
    std::ofstream run_cfg((log_dir / "run_config.json").string(),
                          std::ios::trunc);
    if (run_cfg.is_open()) {
        run_cfg
            << "{\n"
    ...
    if (log.is_open() && log.tellp() == 0) {
        log << "run_id,timestamp,l0_files,stall_micros_total,stall_micros_delta,compaction_pending_bytes,write_throughput_mb_s,batch_latency_ms,bg_jobs\n";
    }
    ...
    for (; !g_stop.load() && (max_rounds == 0 || rounds_done < max_rounds);
        ...
        int bg_jobs = controller_enabled ? ctl.CurrentBgJobs() : 0;
